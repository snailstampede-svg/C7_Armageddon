...
resource "aws_acm_certificate" "NV-cert" {
...
resource "aws_lb" "shinjuku_alb01" {
  ...
  depends_on      = [aws_s3_bucket_policy.shinjuku_alb_logs_policy01]
  # TODO: students can enable access logs to S3 as a stretch goal
  access_logs {
    bucket  = aws_s3_bucket.shinjuku_alb_logs_bucket91[0].bucket
...
resource "aws_lb_listener" "shinjuku_https_listener01" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "Access Denied: Origin is Cloaked"
      status_code  = "403"
    }
  }
}
