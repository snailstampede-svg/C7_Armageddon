...
# 2. Static Entrypoint (Modified for Lab 2B-Be-A-Man-C)
@app.route('/static/index.html')
def serve_index():
    resp = send_from_directory(STATIC_DIR, 'index.html')
    
    # REQUIRED: Set a short TTL to trigger expiration for testing [cite: 13]
    resp.headers['Cache-Control'] = 'public, max-age=10'
    
    # REQUIRED: Set a manual ETag validator 
    # This allows CloudFront to perform the RefreshHit check 
    resp.headers['ETag'] = '"snailtek-v1.0.0"' 
    
    return resp

@app.route('/static/<path:path>')
def serve_static(path):
    # Standard static server for other assets [cite: 11]
