...
resource "aws_internet_gateway" "lab_1c_igw01" {
...
resource "aws_subnet" "lab_1c_public_subnets" {
...
resource "aws_subnet" "lab_1c_private_subnets" {
...
# resource "aws_eip" "lab_1c_nat_eip01" {
#   domain = "vpc"
...
# resource "aws_nat_gateway" "lab_1c_nat01" {
#   allocation_id = aws_eip.lab_1c_nat_eip01.id
#   subnet_id     = aws_subnet.lab_1c_public_subnets[0].id # NAT in a public subnet

#   tags = {
#     Name = "${local.name_prefix}-nat01"
#   }

#   depends_on = [aws_internet_gateway.lab_1c_igw01]
# }
...
resource "aws_route_table" "lab_1c_public_rt01" {
  vpc_id = aws_vpc.lab_1c_vpc01.id
...
resource "aws_route" "lab_1c_public_default_route" {
  route_table_id         = aws_route_table.lab_1c_public_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.lab_1c_igw01.id
}

# Explanation: Attach public subnets to the “public lanes.”
resource "aws_route_table_association" "lab_1c_public_rta" {
  count          = length(aws_subnet.lab_1c_public_subnets)
  subnet_id      = aws_subnet.lab_1c_public_subnets[count.index].id
  route_table_id = aws_route_table.lab_1c_public_rt01.id
}

# Explanation: Private route table = “stay hidden, but still ship supplies.”
resource "aws_route_table" "lab_1c_private_rt01" {
  vpc_id = aws_vpc.lab_1c_vpc01.id
...
# resource "aws_route" "lab_1c_private_default_route" {
#   route_table_id         = aws_route_table.lab_1c_private_rt01.id
#   destination_cidr_block = "0.0.0.0/0"
#   nat_gateway_id         = aws_nat_gateway.lab_1c_nat01.id
# }

# Explanation: Attach private subnets to the “stealth lanes.”
resource "aws_route_table_association" "lab_1c_private_rta" {
  count          = length(aws_subnet.lab_1c_private_subnets)
  subnet_id      = aws_subnet.lab_1c_private_subnets[count.index].id
  route_table_id = aws_route_table.lab_1c_private_rt01.id
}
...
resource "aws_security_group" "lab_1c_ec2_sg01" {
...
resource "aws_security_group" "lab_1c_vpce_sg01" {
  ...
  ingress {
    ...
    to_port         = 443
    protocol        = "tcp"
    security_groups = [aws_security_group.lab_1c_ec2_sg01.id]
  }
}
...
resource "aws_security_group" "lab_1c_rds_sg01" {
...
resource "aws_vpc_endpoint" "s3_gw" {
  ...
  route_table_ids   = [aws_route_table.lab_1c_private_rt01.id]
}
...
resource "aws_vpc_endpoint" "ssm" {
  ...
  subnet_ids          = aws_subnet.lab_1c_private_subnets[*].id
  security_group_ids  = [aws_security_group.lab_1c_vpce_sg01.id]
}
# Explanation: ec2messages is the Wookiee messenger—SSM sessions won’t work without it.
resource "aws_vpc_endpoint" "lab_1c_vpce_ec2messages01" {
  vpc_id              = aws_vpc.lab_1c_vpc01.id
...
resource "aws_vpc_endpoint" "lab_1c_vpce_ssmmessages01" {
  vpc_id              = aws_vpc.lab_1c_vpc01.id
...
resource "aws_vpc_endpoint" "lab_1c_vpce_logs01" {
  vpc_id              = aws_vpc.lab_1c_vpc01.id
  ...
  subnet_ids         = aws_subnet.lab_1c_private_subnets[*].id
  security_group_ids = [aws_security_group.lab_1c_ec2_sg01.id]
...
resource "aws_db_subnet_group" "lab_1c_rds_subnet_group01" {
...
resource "aws_db_instance" "lab1crds" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.lab_1c_rds_subnet_group01.name
  vpc_security_group_ids = [aws_security_group.lab_1c_rds_sg01.id]
...
resource "aws_iam_role" "lab_1c_ec2_role01" {
...
resource "aws_iam_policy" "lab_1c_leastpriv_read_params01" {
...
resource "aws_iam_policy" "lab_1c_leastpriv_read_secret01" {
...
resource "aws_iam_policy" "lab_1c_leastpriv_cwlogs01" {
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_cloudwatch_log_group.lab_1c_log_group01.arn}:*"
        ]
      }
    ]
  })
...
resource "aws_iam_role_policy_attachment" "lab_1c_ec2_ssm_attach" {
  role       = aws_iam_role.lab_1c_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

# Explanation: EC2 must read secrets/params during recovery—give it access (students should scope it down).
resource "aws_iam_role_policy_attachment" "lab_1c_ec2_secrets_attach" {
  role       = aws_iam_role.lab_1c_ec2_role01.name
...
resource "aws_iam_role_policy_attachment" "lab_1c_ec2_cw_attach" {
  role       = aws_iam_role.lab_1c_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}

# Explanation: Instance profile is the harness that straps the role onto the EC2 like bandolier ammo.
resource "aws_iam_instance_profile" "lab_1c_instance_profile01" {
...
# resource "aws_instance" "lab_1c_ec201" {
#   ami                    = var.ec2_ami_id
#   instance_type          = var.ec2_instance_type
#   subnet_id              = aws_subnet.lab_1c_public_subnets[0].id
#   vpc_security_group_ids = [aws_security_group.lab_1c_ec2_sg01.id]
#   iam_instance_profile   = aws_iam_instance_profile.lab_1c_instance_profile01.name
#   user_data = templatefile("${path.module}/user_data.sh", {
#     region         = var.aws_region # Make sure var.aws_region is set to "ap-southeast-7"
#     secret_id      = aws_secretsmanager_secret.lab_1c_db_secret01.name
#     log_group_name = aws_cloudwatch_log_group.lab_1c_log_group01.name
#   })
...
resource "aws_instance" "lab_1c_ec201" {
  ami                    = var.ec2_ami_id
  instance_type          = var.ec2_instance_type
  subnet_id              = aws_subnet.lab_1c_private_subnets[0].id # MOVED TO PRIVATE 
  vpc_security_group_ids = [aws_security_group.lab_1c_ec2_sg01.id]
  iam_instance_profile   = aws_iam_instance_profile.lab_1c_instance_profile01.name
...
resource "aws_ssm_parameter" "lab_1c_db_endpoint_param" {
...
resource "aws_ssm_parameter" "lab_1c_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
...
resource "aws_ssm_parameter" "lab_1c_db_name_param" {
  name  = "/lab/db/name"
  type  = "String"
  value = var.db_name
...
resource "aws_secretsmanager_secret" "lab_1c_db_secret01" {
  name = "${local.name_prefix}/rds/mysql_v11"
}

# Explanation: Secret payload—students should align this structure with their app (and support rotation later).
resource "aws_secretsmanager_secret_version" "lab_1c_db_secret_version01" {
  secret_id = aws_secretsmanager_secret.lab_1c_db_secret01.id
...
resource "aws_cloudwatch_log_group" "lab_1c_log_group01" {
...
resource "aws_cloudwatch_metric_alarm" "lab_1c_db_alarm01" {
...
resource "aws_cloudwatch_log_metric_filter" "lab_1c_db_error_filter" {
...
resource "aws_sns_topic" "lab_1c_sns_topic01" {
...
resource "aws_sns_topic_subscription" "lab_1c_sns_sub01" {
  topic_arn = aws_sns_topic.lab_1c_sns_topic01.arn
  protocol  = "email"
